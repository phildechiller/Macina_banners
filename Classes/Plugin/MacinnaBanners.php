 ...
 */
class MacinaBanners extends AbstractPlugin
{
    ...
    function main($content, $conf)
    {
        ...
        if ($this->piVars['banneruid']) {
			...
			$queryBuilder = GeneralUtility::makeInstance(ConnectionPool::class)->getQueryBuilderForTable('tx_macinabanners_banners');
            if ($record != false) {
                // update clicks
				$this->getDatabaseConnection('tx_macinabanners_banners')
				->update('tx_macinabanners_banners') // table
				->where($this->getDatabaseConnection('tx_macinabanners_banners')->expr()->eq('uid',$record['uid']))
				->set('clicks',++$record['clicks'])
				->execute();
    ...
    function listView($content, array $conf)
    {
		...
		$database = $this->getDatabaseConnection('tx_macinabanners_banners');
        // passende sprache oder sprachunabhaengig
        
		// Wird nicht für unterschiedliche Sprachen benutzt. Einheitlich auf Website
		//$where = '(sys_language_uid = ' . $GLOBALS['TSFE']->sys_language_uid . ' OR sys_language_uid = -1)';

		// Wird in neuer API bereits hinzugefügt
            ...
            */
            foreach ($allowedPlacements as $key => $placement) {
                if (GeneralUtility::inList('top,bottom,right,left', $placement)) {
                   ...
                   // $catWhere = '\'%' . $placement . '%\'';
                    $catRS = $this->getDatabaseConnection('tx_macinabanners_categories')->select('*')->from('tx_macinabanners_categories')->where(
					$this->getDatabaseConnection('tx_macinabanners_categories')->expr()->like(
					'description',
					'\'%' . $this->getDatabaseConnection('tx_macinabanners_categories')->escapeLikeWildcards($placement) . '%\''
					)
					)->execute();
					
					//\TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($catRS->fetch());
                    $catRow = $catRS->fetch();
					$allowedPlacements[$key] = '\'tx_macinabanners_categories:'.$catRow['uid'].'\'';
					
                    
                }
				//\TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($allowedPlacements);
                //$placementClauses[] = $queryBuilder->expr()->inSet('placement', $allowedPlacements[$key]);
            }
            //$where .= ' AND ' . implode(' OR ', $placementClauses);
        }

		//Banner nur anzeigen, wenn die Kategorie überein stimmt
		//$database->where($database->expr()->in('placement',$database->createNamedParameter($allowedPlacements,Connection::PARAM_STR_ARRAY)));
		
        // alle banner die die aktuelle page id nicht in excludepages stehen haben
		// Wird nicht benötigt, da beim ListView die Ordner ausgewählt werden -> Wieso hier nochmal unterscheiden?
        ...
        // if (!empty($conf['pidList'])) {
            ...
            // }
        // }
		//\TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($conf['pidList']);
		// Alle Banner aus einer bestimmten PID anzeigen
		if(!empty($conf['pidList']))
		{
			$pidList = GeneralUtility::intExplode(',',$conf['pidList']);
		}
		else
		{
			$pidList = $conf['pidList'];
		}
		 
		//$database->where($database->expr()->in('pid',$database->createNamedParameter($pidList,Connection::PARAM_INT_ARRAY)));
        ...
        if (isset($conf['enableParameterRestriction']) && !empty($conf['enableParameterRestriction'])) {
			...
			$res = $database->select('uid','parameters')->from('tx_macinabanners_banners')
			->where(
				$database->expr()->in('pid',$database->createNamedParameter($pidList,Connection::PARAM_INT_ARRAY)),
				$database->expr()->in('placement',$database->createNamedParameter($allowedPlacements,Connection::PARAM_STR_ARRAY))
			)
			->groupBy($orderBy);
			//->execute();
            ...
            if ($ids != '') {
                $res = $this->getDatabaseConnection('tx_macinabanners_banners')->select('*')->from('tx_macinabanners_banners')->where($database->expr()->in('uid',$database->createNamedParameter($ids,Connection::PARAM_INT_ARRAY)))->execute();
            } else {
			...
			$res = $database->select('*')->from('tx_macinabanners_banners')
			->where(
				// $database->expr()->in('pid',$database->createNamedParameter($pidList,Connection::PARAM_INT_ARRAY)),
				// $database->expr()->in('placement',$database->createNamedParameter($allowedPlacements,Connection::PARAM_STR_ARRAY))
				$database->expr()->in('pid',$pidList),
				$database->expr()->in('placement',$allowedPlacements)
			)
			->groupBy($orderBy)
			->execute();
			// \TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($res->fetch());
        }
        ...
        // banner aussortieren
        $bannerdata = [];
        while ($queryPerformed && $row = $res->fetch()) {
        ...
        for ($i = 0; $i < $qt; $i++) {
			...
			$this->getDatabaseConnection('tx_macinabanners_banners')
			->update('tx_macinabanners_banners') // table
			->where($this->getDatabaseConnection('tx_macinabanners_banners')->expr()->eq('uid',$row['uid']))
